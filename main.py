...
def generate_answer(question):
     ...
     if image:
         ...
         if payment:
             ...
             invitation.send_messages_to_bord_members(image,text)   
     else:
         question=question.lower()
         response = palm.chat(messages=question, temperature=0.7, context="you are assistance bot give 5 lines")
         if any(keyword in response.messages[0]['content'] for keyword in key_words_generation):
             text="Please tell the Date in for YY/MM/DD of report generation"
             return text  
         elif re.search(pattern, response.messages[0]['content'], re.IGNORECASE):
            ...
            send_message.send_text_to_board_members(response.message[0]["content"])
             ...
             send_message=President_send_message()
             send_message.send_text_to_board_members(response.message[0]["content"])
         elif any(keyword in response.messages[0]['content'] for keyword in key_words_Donation):
             ...
             return text
         elif any(keyword in response.messages[0]['content'] for keyword in key_words_Dues):
