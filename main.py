...
from pdf_pipe_line import Pdf_Pipeline_Config
from profile_generation import PdfPipelineConfig_profile
...
def generate_answer(question,phone_number):
        ...
        pattern = re.compile(r'^verified (\d{12}) trust account$', re.IGNORECASE)
        key_words_profile_generation=["profile","i need my profile"]
        ...
        if any(keyword in response.messages[0]['content'].lower() for keyword in key_words_generation):
            ...
            pdf_path=config.main(start_date1,start_date2,save_path_base)
            return pdf_path
        elif any(keyword in response.messages[0]['content'].lower() for keyword in key_words_profile_generation):
            file_path_profile=r"Data\Members profile sample.pdf"
            scannedPdfConverter(file_path_profile,save_path)
            profile_config=PdfPipelineConfig_profile()
            pdf_path=profile_config.main("9791339999",save_path)
            return pdf_path
        elif re.search(pattern, response.messages[0]['content']):
            scannedPdfConverter(file_path_profile,save_path)
            if phone_number=="918861832522":
                config=Pdf_Pipeline_Config()
                pdf_path=config.main(response.messages[0]['content'],save_path)
                print(pdf_path)
                return pdf_path
